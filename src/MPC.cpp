...
// Lf使用经验值，该值是模拟器中汽车在平坦地面上以恒定的速度与转角运动，
// 得到的车辆行驶轨迹的半径。
...
size_t a_start = delta_start+N-1; // delta的个数注意为N-1
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for(unsigned int i=0; i<N; i++){
      ...
      // 该可显著影响其过弯速度，但回到直线后会出现误差累计的现象（运动不稳定）
